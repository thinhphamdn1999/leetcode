function visit(columnIndex, rowIndex, columnLength, rowLength, check, orderedArr, matrix) {
...
function spiralOrder(matrix) {
    const result = [];
    const check = Array.from({ length: matrix.length }, () => Array.from({ length: matrix[0].length }, () => 0));
