function isValidSudoku(board: string[][]): boolean {
	const columnMaps = Array.from({ length: 9 }, () => new Map<string, number>());
	const rowMaps = Array.from({ length: 9 }, () => new Map<string, number>());
	const blockMaps = Array.from({ length: 3 }, () =>
		Array.from({ length: 3 }, () => new Map<string, number>())
	);
